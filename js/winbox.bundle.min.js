...
(function() {
    ...
    function V(a, b) {
        ...
        E ? ((l ||
            z)
            .appendChild(this.body), n = Math.max(Math.min(this.body.clientWidth + 2 * O + 1, u), C), q = Math.max(Math.min(this.body.clientHeight + this.h + O + 1, v), D), this.g.appendChild(this.body)) : (n = n ? W(n, u) : Math.max(u / 2, C) | 0, q = q ? W(q, v) : Math.max(v / 2, D) | 0);
        Y = Y ? W(Y, e, n) : N;
        Z = Z ? W(Z, t, q) : M;
        this.x = Y;
        this.y = Z;
        ...
        this.onhide = wa;
        this.onshow = xa;
        H ? this.maximize() : G ? this.minimize() : this.resize()
            .move();
        if (I) this.hide();
    ...
    function W(a, b, c) {
        "string" === typeof a && ("center" === a ? a = (b - c) / 2 | 0 : "right" === a || "bottom" === a ? a = b - c : (c = parseFloat(a), a = "%" === ("" + c !== a && a.substring(("" + c)
            .length)) ? b / 100 * c | 0 : c));
        return a
    }
    ...
    function la() {
        ...
        e = 0;
        for (var g, l; e < a; e++) d = x[e], g = (d.left || d.right) + ":" + (d.top || d.bottom), l = Math.min((P - d.left - d.right) / c[g], 250), d.resize(l + 1 | 0, d.h, !0)
            .move(d.left + b[g] * l | 0, Q - d.bottom - d.h, !0), b[g]++
    }
    ...
    function X(a, b) {
        ...
        function e(h) {
            ...
            E && (a.y = Math.max(Math.min(a.y,
                Q - a.height - a.bottom), a.top), E = a.y !== v);
            (G || E) && a.move();
            if (H || G) F = B;
            if (I || E) t = h
        }
    ...
    f.restore = function() {
        J && za();
        this.min && (ma(this), this.resize()
            .move(), this.onrestore && this.onrestore());
        this.max && (this.max = !1, this.removeClass("max")
            .resize()
            .move(), this.onrestore && this.onrestore());
        return this
    };
    f.maximize = function(a) {
        ...
        J && za();
        this.min && ma(this);
        this.max || (this.addClass("max")
            .resize(P - this.left - this.right, Q - this.top - this.bottom, !0)
            .move(this.left, this.top, !0), this.max = !0, this.onmaximize && this.onmaximize());
        return this
    };
